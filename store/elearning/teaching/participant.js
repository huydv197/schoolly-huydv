...
const actions = {
  ...
  async [actionTypes.ELEARNING_TEACHING_EXERCISE_PARTICIPANT.QUESTION_SUBMISSION_FILES]({ commit }, payload) {
    try {
      const result = await this.$axios.get(
        `${APIs.ELEARNING_TEACHING_SUBMISSIONS_EXPORT}`,
        {
          responseType: "arraybuffer",
          params: payload
        },
      ...
      console.log("[Teaching question submission] download.error", error);
    }
  },
};
