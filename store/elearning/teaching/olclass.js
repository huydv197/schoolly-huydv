...
import OlClassNew from "~/services/elearning/mytutor/Olclass";
...
const actions = {
  ...
  async [actionTypes.TEACHING_OLCLASSES.ADD]({ commit }, payload) {
    try {
      const result = await new OlClassNew(this.$axios)["postWithRawJson"](payload);
      return result;
    } catch (error) {
  ...
  async [actionTypes.TEACHING_OLCLASSES.EDIT]({ commit }, payload) {
    try {
      const result = await new OlClassNew(this.$axios)['putWithRawJson'](payload);
      return result;
    } catch (error) {
  ...
  async [actionTypes.TEACHING_OLCLASSES.INFO]({ commit }, id) {
    try {
      const result = await new OlClassNew(this.$axios)[actionTypes.BASE.DETAIL](
        id
      );
      commit(
