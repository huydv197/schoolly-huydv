<template>
  <div class="container mytutor-olclass">
    ...
    <div class="row">
      ...
      <div class="col-md-9">
        <sub-block-section class="mb-4" hasIcon title="Sửa lớp học online">
        ...
        <div class="text-right">
          <app-button @click.prevent="handleEditClass">
            <IconDownloadDone class="icon fill-opacity-1 body-1 mr-2" />Cập nhật
          </app-button>
        </div>
      </div>
    </div>
...
export default {
  ...
  async fetch({ params, query, store, route }) {
    ...
    await Promise.all([
      store.dispatch(`elearning/mytutor/olclass/${actionTypes.MYTUTOR_OLCLASS.LIST_DETAIL}`, requestId),
      store.dispatch(
  ...
  methods: {
    ...
    cloneDataEdit(){
      const dataClone = {...this.olClassListDetail}
      this.params.id = this.$route.params.id ? this.$route.params.id : "";
      this.params.name = dataClone.name;
      this.params.description = dataClone.description;
      this.params.assign_class_ids = dataClone.assign_classes && dataClone.assign_classes.map(item => {
        return item.id
      }) || [];
      const concat1 = dataClone.assign_classes && dataClone.assign_classes.map(item => {
        return item.name
      }) || [];
      this.params.assign_group_ids = dataClone.assign_groups && dataClone.assign_groups.map(item => {
        return item.id
      }) || [];
      const concat2 = dataClone.assign_groups && dataClone.assign_groups.map(item => {
        return item.name
      }) || [];
      this.params.fee_type = dataClone.fee_type;
      this.params.assignment_type = dataClone.assignment_type;
      this.params.grade_id = dataClone.grade_id;
      this.grade = dataClone.grade_id;
      this.params.subject_id = dataClone.subject_id;
      this.subject = parseInt(dataClone.subject_id);
      this.params.fee = dataClone.fee;
      this.params.privacy = dataClone.privacy;
      this.filterClassGroup = concat1.concat(concat2);
      this.params.is_invite_all = dataClone.is_invite_all;
      this.params.is_allow_download = dataClone.is_allow_download;
      this.schedules = dataClone.schedules.map(item => {
        return {
          ...
          start_time: this.convertStart(item.start_time),
          to_date: item.to_date,

        }
      });
      this.params.schedules = dataClone.schedules;
    },
    ...
    handleChangeSubject(_val) {
      // console.log("handleChangeSubject", _val, typeof _val);
      this.params.subject_id = _val;
    },
    ...
    handleRemoveSchedule(index) {
      // console.log("handleRemoveSchedule");
    ...
    async handleEditClass() {
      try {
        ...
        const doCreate = await this.$store.dispatch(
          `elearning/mytutor/olclass/${actionTypes.MYTUTOR_OLCLASS.EDIT}`,
