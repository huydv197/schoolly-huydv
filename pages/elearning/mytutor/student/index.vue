...
const STORE_NAMESPACE_MYTUTOR_STUDENT = "elearning/mytutor/student";
export default {
  components: {
  ...
  async fetch({ params, query, store, route }) {
    ...
    await Promise.all([
      store.dispatch(
        `${STORE_NAMESPACE_MYTUTOR_STUDENT}/${actionTypes.OLCLASS_JOINED_STUDENTS.LIST}`,
        {
          page: 1,
          size: 10,
        }
  ...
  computed: {
    ...mapState(STORE_NAMESPACE_MYTUTOR_STUDENT, ["studentOlClass"]),
    // ...mapState("elearning/students", ["teachingClgrStudentList"]),
    // ...mapState("elearning", ["currentGroupCls"]),
    filterClgrStudentList() {
      const dataStudentList = get(this, "studentOlClass.data.content", []);
    ...
    filterPagination() {
      return {
        size: get(this, "studentOlClass.data.size", 10),
        total_pages: get(this, "studentOlClass.data.total_pages", 1),
        total_elements: get(this, "studentOlClass.data.total_elements", 0),
        first: get(this, "studentOlClass.data.first", 1),
        last: get(this, "studentOlClass.data.last", 1),
        number_of_elements: get(
          this,
          "studentOlClass.data.number_of_elements",
          0
        ),
        number: get(this, "studentOlClass.data.number", 0),
      };
    },
  },
  created() {
  ...
  methods: {
    get,
    getDateBirthDayUTC,
    ...mapActions(STORE_NAMESPACE_MYTUTOR_STUDENT, [
      "getOlClassJoinedStudents",
    ]),
    getListOlClassJoinedStudents() {
      this.getOlClassJoinedStudents(this.paramStudent);
    },
    onPageChange(e) {
      this.paramStudent.page = e && e.number ? e.number + 1 : 1;
      this.getListOlClassJoinedStudents();
    },
