...
export default {
  ...
  methods: {
    ...
    async initSocket() {
      ...
      if (_socket || this.stateSocket) {
        this.socket = _socket ? _socket : this.stateSocket;
      } else {
      ...
      this.socket.on(constants.CHAT.MESSAGE, (data) => {
        console.log("[socket] messageOn1", data, this.socket);
  ...
  watch: {
    messageEmit(_newVal) {
      if (_newVal) {
        // const uuidV4 = uuidv4();
        console.log("_newVal messageEmit", _newVal);
        const paramsMessage = {
        ...
        this.socket.emit(constants.CHAT.MESSAGE, paramsMessage, (res) => {
          console.log("[socket] emit ack", res, res.data, this.socket);
          if (res) {
            // this.getRoomList({ reviceMessage: true });
