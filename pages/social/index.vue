...
export default {
  ...
  beforeRouteUpdate (to, from, next) {
    if (process.browser) {
      // Check is routing from a social page to post detail page,
      // then show modal detail instead of routing to post detail page
      const isFromSocial =
        window.location.pathname.indexOf("/social") === 0 ||
        from.path.indexOf("/social") === 0;
      const isToPostDetail =
        to.path.indexOf("/social/post/") === 0 &&
        to.query.attachment_id &&
        to.query.attachment_type;

      if (isFromSocial && isToPostDetail) next(from);
      else next();
    } else next();
  },
