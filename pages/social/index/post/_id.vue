<template>
  <main class="page-social-post-detail">
    ...
    <template v-else>
      <a ref="postCommentScroll" v-scroll-to="'#post__comment-editor'" style="display: none"></a>
      <Post
        :post="post"
...
export default {
  ...
  watch: {
    ...
    "$fetchState.pending": function (newVal) {
      if (!newVal) {
        const { attachment_id, attachment_type } = this.$route.query || {};
        if (attachment_id && attachment_type) {
          this.showModalOnInital({
  ...
  methods: {
     ...
     */
    showModalOnInital({ attachmentId, attachmentType } = {}) {
      if (!attachmentId || !attachmentType) return;
      const { attachments = [] } =
        this.post.type === POST_TYPES.SHARE ? this.post.payload : this.post;
      const photoObj =
      ...
      this.handleClickImage(
        ...
        attachments.length,
        this.post.type === POST_TYPES.SHARE ? this.post.payload : this.post,
        this.post.type === POST_TYPES.SHARE ? this.post : null,
        false
      );
    },
