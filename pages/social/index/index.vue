...
export default {
  ...
  methods: {
    ...
    showModalOnInital(state) {
      ...
      const postIndex = this.feeds.findIndex((p) => p.id === postId);
      
      let post = null;
      if (postIndex > -1) {
        post = this.feeds[postIndex]
      } else {
        const parentSharedPost = this.feeds.find((p) => p.payload && p.payload.id === postId);
      ...
      this.handleClickImage(
        ...
        attachments.length,
        post,
        postIndex > -1 ? null : this.feeds[postIndex],
        false
      );
    },
