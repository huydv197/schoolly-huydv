  ...
  <AccountPhotoWrapper @add-post-success="handleAddPostSuccess">
    ...
    <template v-else>
      <div class="row social-photo-row">
        ...
        >
          <a
            :href="`/social/post/${attachment.post_id}?attachment_id=${attachment.id}`"
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (typeof window.history.pushState != "undefined") {
          window.history.pushState(
            { theater: true },
            "",
            `${window.location.origin}/social/post/${attachment.post_id}?attachment_id=${attachment.id}&attachment_type=${attachment.type}`
          );
        } else {
          this.$router.push(
            `social/post/${attachment.post_id}?attachment_id=${attachment.id}&attachment_type=${attachment.type}`
          );
          return;
        }
     ...
     */
    async deleteChildCommentDetailPost({ id, parentPostId, postId, parentCommentId }) {
      const doDelete = await this.$store.dispatch(
