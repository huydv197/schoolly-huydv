    ...
    >
      <template v-for="post in posts">
        <Post
          ...
          :key="post.id"
          :post="post"
          :author-id="post.$parentId ? get(post, 'author.id') : socialProfile.id"
          :author-name="post.$parentId ? get(post, 'author.fullname') : socialProfile.fullname"
          :author-avatar="post.$parentId ? get(post, 'author.avatar.low') : socialProfile.avatar ? socialProfile.avatar.low : ''"
          :show-menu-dropdown="post.$parentId ? true : post.created_by === uuidUser"
          :user-uuid="uuidUser"
...
export default {
  ...
  methods: {
    ...
    /**
     * To manual show modal detail post
     */
    showModalOnInital(state) {
      const query = getAllUrlParams();
      const { postId, attachmentId, attachmentType } = state;
      ...
      if (postIndex > -1) {
        ...
        const parentSharedPost = this.posts.find((p) => p.payload && p.payload.id === postId);
        parentSharedPost && (post = parentSharedPost.payload || {})
      }

      const { attachments = [] } = post;
      const photoObj =
        attachments && attachments.find((item) => item.id === attachmentId);
      if (!photoObj) return;
      const photoIndex =
        attachments &&
        attachments.findIndex((item) => item.id === attachmentId);

      this.handleClickImage(
        { id: attachmentId, url: photoObj.url, object: attachmentType },
        photoIndex,
        attachments.length,
        post,
        postIndex > -1 ? null : this.posts[postIndex],
        false
      );
    },
    ...
    async handleClickImage(
      ...
      if (
        ...
        window.history[pushState ? "pushState" : "replaceState"](
          ...
          "",
          `${window.location.origin}/social/post/${post.id}?attachment_id=${id}&attachment_type=${object}`
        );
      } else {
        this.$router.push(
          `social/post/${post.id}?attachment_id=${id}&attachment_type=${object}`
        );
      }
    ...
    handlePopstate(event) {
      const timeout = setTimeout(() => {
        if (event.state.theater) {
          this.showModalOnInital(event.state);
        } else {
     ...
     */
    handleCloseModal(pushState = true) {
      if (
        typeof window.history.pushState != "undefined" &&
        typeof window.history.replaceState != "undefined"
      ) {
     ...
     */
    async likeComment(isLiked, post, parentCommentId, commentId, emoji, cb) {
     ...
     */
    async editCommentDetailPost(
      { parentPostId, postId, commentId, content },
      cancelEdit
    ) {
      const model = editComment(content);
    ...
    async postChildCommentDetailPost(
      { content, parentPostId, postId, commentId },
      clearEditor
    ) {
      const model = createComment(content);
      const doAction = await this.$store.dispatch(
    ...
    async editChildCommentDetailPost(
      { parentPostId, postId, commentId, content, parentCommentId },
      cancelEdit
    ) {
      const model = editComment(content);
    ...
    async deleteChildCommentDetailPost({
      ...
      const doDelete = await this.$store.dispatch(
        ...
        { id, parentPostId, postId, parentCommentId }
      );
