<template>
  <div class="post-image" :class="classes">
    <div v-if="images.length > 1" class="post-image-grid">
      <template v-for="(item, index) in images.slice(0, 5)">
        <a
          ...
          :key="item.id"
          :href="`${location}/social/post/${postId}?attachment_id=${item.id}&attachment_type=${item.object}`"
    ...
    <a
      v-else
      class="post-image-item"
      :href="`${location}/social/post/${postId}?attachment_id=${images[0].id}&attachment_type=${images[0].object}`"
