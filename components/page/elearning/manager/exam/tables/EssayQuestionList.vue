      ...
      <template v-slot:cell(action)="{row}">
        <td>
          <p class="text-center w-100">
            ...
            <button
              v-if="getAnswer(row) &&
                get(getAnswer(row), 'upload.length', 0) > 0"
              class="d-inline-block text-primary"
              @click="downdownSubmission(row)"
            >
              Tải về
            </button>
            <span
...
export default {
  ...
  methods: {
    ...
    ...mapActions("elearning/teaching/participant", {
      getQuestionSubmissionFile: 'elearningTeachingSubmissionOfQuestionFileList'
    }),
    ...
    async downdownSubmission(row) {
      try {
        ...
        const fileContent = await this.getQuestionSubmissionFile(
          params
        );
